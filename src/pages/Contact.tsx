...
const Contact = () => {
  ...
  const contactInfo = [
    ...
    },
    {
      icon: Phone,
      label: 'Phone',
      value: '+91 90271 32996',
      href: 'tel:+919027132996',
    },
    {
      icon: MapPin,
      label: 'Location',
  ...
  const inputClass =
    'w-full px-4 py-3 bg-zinc-800/60 border border-zinc-700/60 text-white placeholder-zinc-500 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all text-sm';
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-5 gap-8 lg:gap-12">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {/* Availability badge */}
            <div className="reveal p-5 bg-zinc-900 border border-indigo-500/20 rounded-xl">
              <div className="flex items-center gap-2 mb-2">
                <span className="w-2 h-2 bg-green-400 rounded-full animate-pulse" />
                <span className="text-green-400 text-xs font-medium">Available for opportunities</span>
              </div>
              <p className="text-zinc-400 text-sm leading-relaxed">
                Currently open to full-time software and AI engineering roles. Typical response within 24 hours.
              </p>
            </div>
                ...
                {contactInfo.map((item, i) => {
                  ...
                  return (
                    <a
                      key={i}
                      href={item.href}
                      className="flex items-center gap-4 p-4 bg-zinc-900 border border-zinc-800 hover:border-indigo-500/30 rounded-xl transition-all duration-200 group hover:-translate-y-px"
                    >
                ...
                {socialLinks.map((link, i) => {
                  ...
                  return (
                    <a
                      ...
                      className="flex items-center gap-4 p-4 bg-zinc-900 border border-zinc-800 hover:border-zinc-700 rounded-xl transition-all duration-200 group hover:-translate-y-px"
                    >
                      <div className="w-9 h-9 rounded-lg bg-zinc-800 border border-zinc-700 flex items-center justify-center flex-shrink-0">
                        <Icon className="text-zinc-400 group-hover:text-white transition-colors" size={16} />
                      </div>
                      <div>
          ...
          {/* Right: form */}
          <div className="reveal reveal-d1 lg:col-span-3">
            ...
            {submitted ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-5 bg-zinc-900 border border-zinc-800 rounded-2xl p-7">
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 px-6 py-3.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-medium rounded-xl transition-all duration-200 shadow-lg shadow-indigo-500/20 hover:-translate-y-px"
                >
                  {isSubmitting ? (
